...
func TestVerifyCodeTrieMultiProof2(t *testing.T) {
	...
	generalizedIndexes[1] = baseGeneralizedIndex + 2 // if set to 2 the test fails, if set to 1 it succeeds
	...
	if ok, err := ssz.VerifyMultiproof(rootNode.Hash(), multiProof.Hashes, multiProof.Leaves, multiProof.Indices); !ok || err != nil {
		// Error message: proof is missing required nodes, either 524288 or 524289
